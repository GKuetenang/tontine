...
final class BuildGroupFinancialDashboardAction
{
    public function execute(Group $group, array $filters = []): array
    {
        $query = Transaction::query()
            ->whereHas('session', fn (Builder $query) => $query->where('group_id', $group->id))
        ...
        return [
            'summary' => [
                ...
                'outstanding_loans' => $this->outstandingLoans($group, $filters),
            ],
            'breakdown' => $breakdown,
    ...
    private function outstandingLoans(Group $group, array $filters): string
    {
