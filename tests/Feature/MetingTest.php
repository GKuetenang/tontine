...
test('a contribution starts unpaid', function (): void {
    ...
    expect($contribution)
        ...
        ->status()->toBe(ContributionStatus::Unpaid);
});
...
test('a partial contribution payment changes status to partial', function (): void {
    ...
    expect($contribution->refresh())
        ...
        ->status()->toBe(ContributionStatus::Partial);
});
...
test('multiple payments can fully settle a contribution', function (): void {
    ...
    expect($contribution)
        ...
        ->status()->toBe(ContributionStatus::Paid);

    expect(
        $contribution->transactions()->count(),
    )->toBe(2);
});
...
test('transaction belongs to correct session and membership', function (): void {
    ...
    expect($transaction)
        ->session_id
        ->toBe(
            $contribution->meeting->session_id,
        )
        ->membership_id
        ->toBe(
            $contribution
