    ...
    return (
        <>
            <div className={cn(className,
                'relative border-input border grid place-items-center rounded-md overflow-hidden bg-muted hover:bg-primary/10 hover:text-primary',
                props['aria-invalid'] && 'ring-destructive ring-[3px] bg-destructive/20', hover && 'bg-primary/10 border-ring text-primary ring-ring/50 ring-[3px]')}
            ...
            >
                <input className="absolute inset-0 opacity-0 cursor-pointer z-10" type="file"
                    ...
                    disabled={!!progress}
                    {...props} />
                <UploadIcon size={24} className={cn('text-input', hover && 'text-primary')} />
                {preview && <img
